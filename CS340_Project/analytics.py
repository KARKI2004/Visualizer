...
class VectorAnalyzer:
    ...
    def vector_operations(self, a, b):
        ...
        results = pd.DataFrame({
            ...
            "Result": [dot, angle, orthogonal, unit_a.tolist(), unit_b.tolist(), proj_a_on_b.tolist()]
        })
...
class ProbabilityAnalyzer(VectorAnalyzer):
    ...
    def categorical_analysis(self, col):
        if self.data is None:
            print("No data loaded.")
            return
        vals = self.data[col].unique()
        
        print(f"\nUnique values in {col}: {vals}")
        print(f"2-permutations (first 5): {list(permutations(vals, 2))[:5]}")
        print(f"2-combinations (first 5): {list(combinations(vals, 2))[:5]}")
        os.makedirs("Output", exist_ok=True)
        with open("Output/categorical_analysis.txt", "w") as f:
            f.write(f"Unique values in {col}: {vals}\n")
            f.write(f"2-permutations: {list(permutations(vals, 2))[:5]}\n")
            f.write(f"2-combinations: {list(combinations(vals, 2))[:5]}\n")
